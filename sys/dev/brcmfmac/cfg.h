...
/* IOCTL commands */
#define BRCMF_C_UP	     2	/* round 48 (AP): bring firmware iface up */
#define BRCMF_C_DOWN	     3	/* round 48 (AP): bring firmware iface down */
#define BRCMF_C_SET_INFRA    20
#define BRCMF_C_GET_BSSID    23
#define BRCMF_C_SET_SSID     26
#define BRCMF_C_GET_CHANNEL  29
#define BRCMF_C_SET_BCNPRD   76	/* round 48 (AP): beacon period (TU) */
#define BRCMF_C_SET_DTIMPRD  78	/* round 48 (AP): DTIM period */
#define BRCMF_C_SET_PM	     86
#define BRCMF_C_SET_AP	     118	/* round 48 (AP): 0=STA infra, 1=AP */
...
/*
 * PMK structure.  ROUND 49b: key[] sized to Linux fwil_types.h
 * BRCMF_WSEC_MAX_SAE_PASSWORD_LEN (128) so the C_SET_WSEC_PMK ioctl payload is
 * byte-identical (132 bytes) to what mainline brcmf_set_wsec() sends.
 */
...
#define BRCMF_WSEC_MAX_PSK_LEN 32	/* raw 802.11 PMK bytes (Linux value) */
#define BRCMF_WSEC_PASSPHRASE  (1 << 0)	/* mainline uses this for SAE only */
...
int brcmf_set_security(struct brcmf_softc *sc, uint32_t wsec,
    uint32_t wpa_auth);
int brcmf_ap_set_wpa2(struct brcmf_softc *sc, const uint8_t *ssid,
    uint32_t ssidlen);
