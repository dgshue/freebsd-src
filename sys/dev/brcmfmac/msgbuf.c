...
brcmf_msgbuf_process_event(struct brcmf_softc *sc, struct msgbuf_common_hdr *msg)
{
	...
	case BRCMF_E_DISASSOC_IND:
		...
		/* ROUND 48 (AP): DEAUTH_IND/DISASSOC_IND = client left our AP.
		 * brcmf_ap_sta_event no-ops unless the vap is HOSTAP. */
		if (event_code == BRCMF_E_DEAUTH_IND ||
		    event_code == BRCMF_E_DISASSOC_IND)
