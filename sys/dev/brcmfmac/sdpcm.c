	...
	for (i = 0; i < 300; i++) {
		...
		if (chan == SDPCM_EVENT_CHANNEL)
			...
			if (rflags & BCDC_DCMD_ERROR) {
				/*
				 * ROUND 49c: preserve the raw firmware BCME
				 * status so callers can log the real reason;
				 * the return value is still flattened to EIO.
				 */
				sc->last_fwerr = fwstatus;
...
brcmf_sdpcm_process_event(struct brcmf_softc *sc, uint8_t *data, uint16_t len)
{
	...
	case 16: /* E_LINK */
		...
		/*
		 * ROUND 48 (AP): DEAUTH_IND/DISASSOC_IND also mean a client left
		 * our AP.  brcmf_ap_sta_event no-ops unless the vap is HOSTAP, so
		 * the STA path above is unaffected.
		 */
		if (event_code == 6 || event_code == 12)
			brcmf_ap_sta_event(sc, event_code, event->msg.addr);
		break;
	case 18: /* E_ASSOC_IND -- AP: a client associated */
	case 19: /* E_REASSOC_IND -- AP: a client reassociated */
		brcmf_ap_sta_event(sc, event_code, event->msg.addr);
		break;
