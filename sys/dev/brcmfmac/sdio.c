...
/*
 * Firmware names, in order of preference.  The BCM43456 (e.g. Ampak AP6256
 * on the Orange Pi RV2) is the sibling of the BCM43455 (same 4345 core,
 * chiprev 9 vs 6) and uses the brcmfmac43456-sdio firmware set; fall back
 * to the 43455 set for the original target hardware.
 */
static const char *brcmf_sdio_fw_names[] = {
	"brcmfmac43456-sdio.bin",
	"brcmfmac43455-sdio.bin",
	NULL
};
static const char *brcmf_sdio_nvram_names[] = {
...
/*
 * Get the first available firmware image from a preference-ordered,
 * NULL-terminated name list (firmware(9) falls back to loading raw files
 * from /boot/firmware/<name>).
 */
static const struct firmware *
brcmf_firmware_get_first(struct brcmf_softc *sc, const char **names)
{
	const struct firmware *fw;
	int i;

	for (i = 0; names[i] != NULL; i++) {
		fw = firmware_get(names[i]);
		if (fw != NULL) {
			device_printf(sc->dev, "using firmware image %s\n",
			    names[i]);
			return (fw);
		}
	}
...
brcmf_sdio_clk_enable(struct brcmf_softc *sc, int alp_only)
{
	...
	if (alp_only) {
		...
		/*
		 * ROUND 41: request HT_AVAIL ONLY -- do NOT also set FORCE_HT.
		 * Linux brcmf_sdio_htclk writes clkreq = SBSDIO_HT_AVAIL_REQ
		 * alone and lets the PMU sequence ALP->HT.  Setting FORCE_HT
		 * forces the HT clock on before the PMU has brought its
		 * resources up, which on the 4345/43456 wedges the transition
		 * so HT_AVAIL (0x80) never sets -- the persistent clkval=0x52
		 * (=ALP_AVAIL|HT_AVAIL_REQ|FORCE_HT, no HT_AVAIL) timeout.
		 */
		req = SBSDIO_HT_AVAIL_REQ;
    ...
    void *nvram, uint32_t nvram_len)
{
	 ...
	 * HT clock to complete F2 initialization.  ROUND 41: request
	 * HT_AVAIL_REQ only (no FORCE_HT) -- matches Linux; FORCE_HT wedges
	 * the PMU ALP->HT transition on the 4345/43456 (clkval=0x52 timeout). */
	sdio_write_1(sc->sdio_func1, SBSDIO_FUNC1_CHIPCLKCSR,
	    SBSDIO_HT_AVAIL_REQ, &error);
	{
		uint8_t clkval;
		for (i = 0; i < 100; i++) {
		...
		/*
		 * ROUND 41: make the post-ARM-release HT-clock outcome always
		 * visible (not just under BRCMF_DBG), so we can SEE whether HT
		 * avails here or times out at clkval=0x52 -- the persistent
		 * blocker after the SDIO write path was fixed (r40).
		 */
		device_printf(sc->dev,
		    "post-ARM HT clock: clkval=0x%02x iter=%d %s\n", clkval, i,
		    (clkval & SBSDIO_HT_AVAIL) ? "(HT_AVAIL)" : "(NO HT_AVAIL)");
	}
...
brcmf_sdio_attach(struct brcmf_softc *sc)
{
	...
	/* Load firmware */
	fw = brcmf_firmware_get_first(sc, brcmf_sdio_fw_names);
	if (fw == NULL) {
		device_printf(sc->dev, "failed to load firmware (%s)\n",
		    brcmf_sdio_fw_names[0]);
		return (ENOENT);
	}
	device_printf(sc->dev, "loaded firmware (%zu bytes)\n",
	    fw->datasize);

	/* Load NVRAM */
	nvram_fw = brcmf_firmware_get_first(sc, brcmf_sdio_nvram_names);
	if (nvram_fw != NULL) {
